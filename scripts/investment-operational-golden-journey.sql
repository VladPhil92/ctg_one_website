...
-- Runs entirely inside the ephemeral CI database and rolls back.
--
-- Certified loop:
-- funding -> authoritative payment -> allocation -> production -> serialization ->
-- inventory -> sale -> customer return/credit note -> settlement -> participant credit ->
-- reinvestment + withdrawal -> approved allocation + confirmed payout -> zero residual balance.
...
-- ---------------------------------------------------------------------------
-- Source lot: creation and funding opening.
-- ---------------------------------------------------------------------------
...
select public.transition_lot_status(:'source_lot_id'::uuid,'FUNDING_PENDING','CI operational journey',null);
select public.transition_lot_status(:'source_lot_id'::uuid,'FUNDING_OPEN','CI operational journey',null);
...
-- ---------------------------------------------------------------------------
-- Participant order + trusted proof + authoritative bank verification.
-- Capital = (1000 + 100 + 50) * 2 bottles/case * 2 cases = 4600 cents.
-- ---------------------------------------------------------------------------
set local role authenticated;
...
select public.record_lot_financial_entry(
  :'source_lot_id'::uuid,'PRODUCTION_COST',4600::bigint,'CI operational authoritative production cost'
);
reset role;
...
select pg_temp.assert_journey(
  ...
  'authoritative payment must activate exactly one allocation'
);
select pg_temp.assert_journey(
  (select count(*)=1 and sum(amount_cents)=4600 from public.investment_payment_receipts where order_id=:'order_id'::uuid),
  'authoritative payment receipt must equal order capital'
);

-- ---------------------------------------------------------------------------
-- Physical lot lifecycle and serialization.
-- ---------------------------------------------------------------------------
set local role authenticated;
select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000712',true);
...
select public.update_bottle_units_status(
  :'source_lot_id'::uuid,
  array(select serial_code from public.investment_bottle_units where lot_id=:'source_lot_id'::uuid order by unit_number),
  'IN_MARKET','CI_JOURNEY_SALES'
);
select public.transition_lot_status(:'source_lot_id'::uuid,'SELLING',null,null);
reset role;

-- ---------------------------------------------------------------------------
-- Sale of all four bottles, then one exact customer return.
-- Gross sale 12000, tax 800. One return reverses gross 3000 and tax 200.
-- ---------------------------------------------------------------------------
set local role authenticated;
...
select pg_temp.assert_journey(
  ...
  'returned bottle must leave customer custody'
);
select pg_temp.assert_journey(
...
-- ---------------------------------------------------------------------------
-- Settlement after commercial close.
-- NDLP = (12000-3000) - (800-200) - 4600 = 3800.
-- Participant owns 100% of eligible cases and receives 50% profit + capital:
-- 4600 + 1900 = 6500 settlement credit.
-- ---------------------------------------------------------------------------
set local role authenticated;
select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000712',true);
...
select pg_temp.assert_journey(
  ...
  'settlement must net sale return and tax reversal before distribution'
);
select pg_temp.assert_journey(
  ...
  'participant settlement credit must equal capital plus formula-pinned profit'
);
select pg_temp.assert_journey(
...
-- ---------------------------------------------------------------------------
-- Target lot and complete post-settlement liquidity split.
-- Reinvestment: (500+50+25) * 2 bottles/case * 2 cases = 2300.
-- Withdrawal: remaining 4200. After both settle, participant balance = 0.
-- ---------------------------------------------------------------------------
set local role authenticated;
select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000712',true);
select public.create_production_lot_from_style(
...
select public.transition_lot_status(:'target_lot_id'::uuid,'FUNDING_PENDING','CI operational reinvestment target',null);
select public.transition_lot_status(:'target_lot_id'::uuid,'FUNDING_OPEN','CI operational reinvestment target',null);
reset role;
...
select pg_temp.assert_journey(
  ...
  'pending reinvestment and withdrawal must reserve the entire settlement credit without double spend'
);
...
select public.initiate_investment_payout(
  ...
  'ci-operational-destination-7711','ci-operational-payout-0001','CI operational journey payout'
) as payout_id \gset
select public.confirm_investment_payout(
  :'payout_id'::uuid,'CI-PAYOUT-CONFIRMED-001',now(),'CI operational journey confirmed'
);
reset role;
...
select pg_temp.assert_journey(
  ...
  'reinvestment approval must preserve participant case intent and server price'
);
select pg_temp.assert_journey(
...
select pg_temp.assert_journey(
  (select count(*)=1 and amount_cents=-4200 and source_payout_id=:'payout_id'::uuid
   from public.investment_ledger_entries where entry_type='WITHDRAWAL_DEBIT' and participant_user_id='00000000-0000-0000-0000-000000000711'),
  'withdrawal debit must retain authoritative payout genealogy'
);
select pg_temp.assert_journey(
  (select count(*)=1 and amount_cents=-2300
   from public.investment_ledger_entries where entry_type='REINVESTMENT_DEBIT' and reference=:'reinvestment_request_id'),
...
select pg_temp.assert_journey(
  ...
  'complete liquidity split must conserve the settlement credit to zero residual balance'
);

-- ---------------------------------------------------------------------------
-- Phase 17 read model must reconstruct the source-lot journey without writes.
-- ---------------------------------------------------------------------------
set local role authenticated;
select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000713',true);
...
select pg_temp.assert_journey((:'journey_snapshot'::jsonb->>'nextAction')='CLOSED_LOOP','journey snapshot must identify closed loop');
select pg_temp.assert_journey((:'journey_snapshot'::jsonb#>>'{settlement,finalized}')::boolean,'journey snapshot must expose finalized settlement');
select pg_temp.assert_journey((:'journey_snapshot'::jsonb#>>'{settlement,participantCreditCents}')::bigint=6500,'journey snapshot must expose exact participant credit');
select pg_temp.assert_journey((:'journey_snapshot'::jsonb#>>'{sales,returnedUnits}')::bigint=1,'journey snapshot must expose the physical/commercial return');
select pg_temp.assert_journey((:'journey_snapshot'::jsonb#>>'{sales,returnGenealogyMismatches}')::bigint=0,'journey snapshot must report return genealogy reconciled');
select pg_temp.assert_journey((:'journey_snapshot'::jsonb#>>'{liquidity,sourceLinkedApprovedReinvestmentCents}')::bigint=2300,'journey snapshot must preserve exact settlement-to-reinvestment genealogy');
select pg_temp.assert_journey((:'journey_snapshot'::jsonb#>>'{liquidity,creditedParticipantWithdrawalCentsAfterSettlement}')::bigint=4200,'journey snapshot must report later withdrawal activity without claiming source attribution');
