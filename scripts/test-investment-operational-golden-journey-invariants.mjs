...
assert.match(migration,/orderBackedAllocationCapitalCents/);
assert.match(migration,/reinvestmentBackedAllocationCapitalCents/);
assert.match(migration,/reinvestmentDebitCents/);
assert.match(migration,/internalAllocationCapitalCents/);
assert.match(migration,/unbackedAllocationCapitalCents/);
assert.match(migration,/rr\.target_lot_id = v_selected/i);
assert.match(migration,/le\.entry_type = 'REINVESTMENT_DEBIT'/i);
assert.match(migration,/v_receipt_cents = v_order_allocation_capital/i);
assert.match(migration,/v_reinvestment_debit_cents = v_reinvestment_allocation_capital/i);
assert.match(migration,/v_unbacked_allocation_capital = 0/i);
...
assert.match(component,/Funding reconciliado/);
assert.match(component,/Reinversión → débito/);
assert.match(component,/Capital sin respaldo/);
