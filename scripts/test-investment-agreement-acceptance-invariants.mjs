...
const [migration, checkoutRepository, checkout, schemaVersion] = await Promise.all([
...
assert.match(migration, /revoke all on function public\.accept_investment_agreement\(\) from public/, 'accept_investment_agreement() must not be public-callable');
assert.match(migration, /grant execute on function public\.accept_investment_agreement\(\) to authenticated/, 'accept_investment_agreement() must be callable by authenticated participants');
...
// otherwise the new gate breaks those scripts for an unrelated reason.
...
console.log('Investment agreement acceptance invariants: PASS');
