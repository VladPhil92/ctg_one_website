...
assert.match(schemaVersion, /EXPECTED_DATABASE_MIGRATION = '0137'/, 'Repository schema authority must reflect the current additive global schema.');
assert.match(schemaVersion, /EXPECTED_DATABASE_MIGRATION_NAME = 'rewards_signed_source_atomicity_hardening'/, 'Current schema authority must name the latest Rewards hardening migration.');
assert.match(schemaVersion, /EXPECTED_DATABASE_MIGRATION_COUNT = 137/, 'Current schema migration count must remain aligned with repository history.');
