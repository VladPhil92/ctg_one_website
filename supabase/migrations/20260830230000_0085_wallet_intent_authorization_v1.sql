...
--   * the trusted CTG One server has independently simulated the exact immutable
--     Polygon transaction and supplied its server-generated SHA-256 evidence digest.
--
...
comment on column public.wallet_intents_v2.simulation_digest_sha256 is
  'SHA-256 digest generated by CTG One from an independently executed server-side Polygon simulation bound to the immutable intent payload.';
...
begin
  ...
  if v_expected_wallet !~ '^0x[0-9a-f]{40}$'
    ...
    raise exception 'WALLET_AUTH_EXPECTED_ADDRESS_INVALID';
  end if;
  if p_expected_chain_id is distinct from 137 then
    raise exception 'WALLET_AUTH_EXPECTED_CHAIN_INVALID';
  end if;
  if v_expected_asset not in ('POL','CTG','USDC','USDT') then
    raise exception 'WALLET_AUTH_EXPECTED_ASSET_INVALID';
  end if;
  if length(v_expected_amount) < 1
     or length(v_expected_amount) > 78
     or v_expected_amount !~ '^[1-9][0-9]*$' then
    raise exception 'WALLET_AUTH_EXPECTED_AMOUNT_INVALID';
  end if;
  ...
  -- Lock the durable request before evaluating either a first authorization or
  -- an idempotent replay. Every field that the trusted simulation used is then
  -- compared to the persisted immutable request to close TOCTOU gaps.
  ...
  if v_intent.chain_id is distinct from p_expected_chain_id
    ...
    raise exception 'WALLET_AUTH_SIMULATION_BINDING_CONFLICT';
  end if;
  ...
  -- A completed durable authorization replays independently of the original
  -- creation TTL. No new authorization occurs, so an expired intent must not
  -- turn a lost-response retry into a false failure.
  if v_intent.status = 'authorized' then
    v_replayed := true;
    if v_intent.authorized_wallet_address is distinct from v_expected_wallet
    ...
    -- Resolve signing identity only from trusted server-owned registry facts for
    -- the first created -> authorized transition. Replay never re-authorizes.
    ...
    if v_primary_wallet.address_normalized is distinct from v_expected_wallet then
      raise exception 'WALLET_AUTH_SIGNER_BINDING_CONFLICT';
    end if;
