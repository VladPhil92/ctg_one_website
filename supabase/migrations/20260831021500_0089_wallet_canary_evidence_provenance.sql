...
--
-- Makes first-real-send evidence durable and auditable without changing
-- signing, broadcast, reconciliation or COP ledger authority.
--
-- Boundary:
--   * one reviewed CTG-Wallet commit can be bound server-side to an already
--     authorized intent before signer access;
--   * every successful non-replayed Polygon reconciliation update is captured
--     in an append-only observation journal;
...
create table if not exists public.wallet_chain_reconciliation_observations_v1 (
  ...
  constraint wallet_chain_reconciliation_observations_v1_intent_digest_unique
    unique (intent_id, evidence_digest_sha256)
);

create index if not exists wallet_chain_reconciliation_observations_v1_intent_sequence_idx
  on public.wallet_chain_reconciliation_observations_v1(intent_id, checked_at, id);
create index if not exists wallet_chain_reconciliation_observations_v1_user_sequence_idx
  on public.wallet_chain_reconciliation_observations_v1(subject_user_id, checked_at desc, id desc);
...
begin
  ...
  if new.chain_last_checked_at is null
     or new.tx_hash is null
     or new.chain_reconciliation_digest_sha256 is null
     or new.status not in ('pending_external','confirmed_external','reconciled','failed') then
    return new;
  end if;
...
-- Direct authoritative intent mutation remains blocked. Provenance binding is a
-- service-role RPC that can touch only metadata while an intent is authorized
-- and has no external transaction state.
