-- JP Valderrama Education Assessment time-consistency hardening
--
-- The scoring RPC already captures clock_timestamp() as v_now, but its entitlement
-- window used transaction-scoped now(). When a free-course entitlement is granted
-- and assessed in the same transaction, starts_at can be later than now(), causing
-- a false EDUCATION_COURSE_ACCESS_REQUIRED. Use the same v_now reference throughout.
