...
test.describe('CTG One authenticated critical journey', () => {
  ...
  test('KYC survives a partial Storage failure and finalizes through the same resumable intake', async ({ page, request }) => {
    ...
    // Provision the isolated test identity directly through the local Auth API.
    // Registration UI validation is covered separately; this journey must stay
    // focused on authenticated login and KYC cross-resource recovery semantics.
