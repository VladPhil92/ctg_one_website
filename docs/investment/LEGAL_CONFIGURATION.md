...
| `maximumAllocationCases` | Implemented, defaults to `null` | No commercial maximum has been decided — this is an *additional* undecided item, not one of the numbered `BR-*` decisions in `BUSINESS_MODEL.md`. `null` means "no configured cap beyond the lot's remaining capacity," the only limit PostgreSQL currently enforces. Override via `CTG_INVESTMENT_MAX_ALLOCATION_CASES` once a business decision exists. |
