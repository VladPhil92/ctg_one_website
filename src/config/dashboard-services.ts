...
export type DashboardServiceStatus = PublicProofStatus | 'ACCOUNT' | 'PILOT' | 'DEVELOPMENT' | 'CONSOLIDATION';
...
export const DASHBOARD_SERVICE_GROUPS: ReadonlyArray<{
  ...
  { id: 'finance', label: 'Capital e identidad', description: 'Wallet, inversión e identidad verificable bajo una sola cuenta CTG One.' },
  { id: 'care', label: 'Aplicaciones y productos', description: 'Productos y experiencias operativas conectadas al ecosistema CTG One.' },
  { id: 'knowledge', label: 'Conocimiento y tecnología', description: 'Inteligencia, activos digitales y capas de conocimiento institucional.' },
  { id: 'education', label: 'Educación', description: 'Contenido académico, servicios educativos y biblioteca personal.' },
] as const;

const investmentStatus = getPublicProofStatus(getCapabilityProof('investment-platform'));
const tokenStatus = getPublicProofStatus(getCapabilityProof('web3'));
const knowledgeStatus = getPublicProofStatus(getCapabilityProof('ctg-knowledge-v01'));
const verticeStatus = getPublicProofStatus(getCapabilityProof('vertice-federation'));
...
export const DASHBOARD_SERVICES: readonly DashboardService[] = [
  ...
  { id: 'pisao', code: 'FNB-12', title: 'PISÁO Gastrobar', description: 'Gastrobar contemporáneo de carácter caribeño y urbano en la Terraza Panorámica del C.C. Mall Plaza Cartagena.', href: 'https://pisaogastrobar.com', cta: 'Explorar PISÁO', group: 'care', status: 'LIVE', publicHref: 'https://pisaogastrobar.com', publicCta: 'Explorar PISÁO' },
  ...
  { id: 'vertice', code: 'CIV-04', title: 'VÉRTICE OS', description: 'Inteligencia ciudadana para reportar, proponer, deliberar, seguir resultados y construir reputación cívica verificable.', href: 'https://vertice.ctgone.com/auth/ctgone/start', cta: 'Entrar con CTG One', group: 'care', status: verticeStatus, serviceKey: 'vertice', publicHref: 'https://vertice.ctgone.com', publicCta: 'Explorar VÉRTICE' },
  { id: 'token', code: 'TKN-06', title: 'CTG One Token', description: 'Consulta la propuesta, utilidad, documentación y evolución del activo digital del ecosistema.', href: '/ctgotoken', cta: 'Explorar Token', group: 'knowledge', status: tokenStatus, serviceKey: 'token', publicHref: '/ctgotoken', publicCta: 'Explorar Token' },
  { id: 'knowledge', code: 'KNW-07', title: 'CTG Knowledge', description: 'Inteligencia institucional basada en conocimiento autorizado, trazable y consultable.', href: '/knowledge', cta: 'Consultar Knowledge', group: 'knowledge', status: knowledgeStatus, serviceKey: 'knowledge', publicHref: '/knowledge', publicCta: 'Consultar Knowledge' },
  ...
  { id: 'rewards', code: 'RWD-11', title: 'CTG Rewards', description: 'Cuenta off-chain e historial auditable de puntos. Foundation v1 está desplegando la infraestructura; acumulación y redención comercial aún no están activas.', href: '/dashboard/rewards', cta: 'Abrir Rewards', group: 'care', status: 'DEVELOPMENT', serviceKey: 'rewards', publicHref: '/rewards', publicCta: 'Ver desarrollo', publicCtaEn: 'View development' },
] as const;
...
export const DASHBOARD_SERVICE_ROUTES: ReadonlyArray<{ prefix: string; serviceKey: FunnelServiceKey }> = DASHBOARD_SERVICES.flatMap((service) => {
  ...
  return [service.href, ...(service.aliases ?? [])].map((prefix) => ({ prefix, serviceKey: service.serviceKey as FunnelServiceKey }));
