...
export const Navbar: React.FC = () => {
  ...
  const platformsLabel = locale === 'es' ? 'Explorar' : 'Explore';
          ...
          <div className="relative z-10 flex min-h-[72px] items-center justify-between gap-5 lg:min-h-[78px]">
            ...
            <div className="hidden min-w-0 flex-1 items-center justify-center gap-4 xl:flex 2xl:gap-7">
              ...
              <div className="relative" ref={platformRootRef}>
                <button type="button" className={`${navLinkClass(isPlatformActive)} gap-1.5 bg-transparent`} aria-expanded={platformsOpen} aria-controls={PLATFORM_MENU_ID} aria-haspopup="menu" onClick={() => setPlatformsOpen((open) => !open)}>
                  {platformsLabel}<ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${platformsOpen ? 'rotate-180' : ''}`} aria-hidden="true" />
                ...
                {platformsOpen && (
                  <div id={PLATFORM_MENU_ID} role="menu" aria-label={platformsLabel} className="absolute left-1/2 top-[calc(100%+18px)] w-72 -translate-x-1/2 overflow-hidden rounded-2xl border border-[#d6ae56]/20 bg-[#050a10]/[.985] p-2.5 shadow-[0_30px_90px_rgba(0,0,0,.55)] backdrop-blur-2xl">
            ...
            <div className="z-50 flex items-center gap-2 xl:hidden">
              <LanguageSwitcher compact />
              <button ref={menuButtonRef} type="button" onClick={() => setIsOpen((open) => !open)} className="flex h-11 w-11 items-center justify-center rounded-xl border border-[#d6ae56]/20 bg-[#050a10]/80" aria-label={isOpen ? closeMenuLabel : openMenuLabel} aria-expanded={isOpen} aria-controls={MOBILE_NAVIGATION_ID}>{isOpen ? <X size={21} className="text-white" aria-hidden="true" /> : <Menu size={21} className="text-text-secondary" aria-hidden="true" />}</button>
            </div>
          </div>
        </div>
      </nav>

      <div id={AFTER_PRIMARY_NAVIGATION_ID} tabIndex={-1} className="h-0 w-full overflow-hidden"><span className="sr-only">{contentStartLabel}</span></div>

      {isOpen && (
        <>
          <button type="button" className="fixed inset-0 z-40 bg-black/80 backdrop-blur-sm xl:hidden" onClick={() => setIsOpen(false)} aria-label={closeMenuLabel} />
          <div id={MOBILE_NAVIGATION_ID} role="dialog" aria-modal="true" aria-label={mobileLabel} className={`${styles.theme} fixed bottom-0 right-0 top-0 z-40 w-[88%] max-w-sm overflow-y-auto border-l border-[#d6ae56]/20 bg-[#050a10]/[.985] xl:hidden`}>
            <div className="relative flex min-h-full flex-col px-6 pb-8 pt-28 sm:px-8 sm:pt-32">
              <div className="mb-7 border-b border-white/[.07] pb-6"><BrandLogo /></div>
