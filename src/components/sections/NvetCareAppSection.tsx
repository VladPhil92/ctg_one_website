...
export const NvetCareAppSection: React.FC = () => {
  ...
  const copy = es
    ? {
        ...
        heroTitle: 'El veterinario que necesitas, donde tu mascota se siente segura.',
        heroText: 'Nvet Care conecta familias con profesionales veterinarios para coordinar atención en casa, acompañar cada visita y mantener el cuidado de la mascota organizado en un solo lugar.',
        ...
        arrivalStatus: 'Seguimiento de la atención',
        petHistory: 'Historial de mascota',
        petHistoryText: 'Una vista continua del cuidado y las visitas.',
        trustStrip: [
          ['Profesionales verificados', 'Identidad y credenciales dentro del flujo de Nvet.'],
          ['Atención en casa', 'Una experiencia diseñada alrededor del bienestar de la mascota.'],
          ['Pagos trazables', 'Flujos de cobro y registro dentro de la plataforma.'],
          ['Historial organizado', 'Cada atención vinculada al perfil de la mascota.'],
        ],
        ...
        steps: [
          ...
          ['02', 'Conecta', 'Nvet organiza el encuentro con un profesional apto para la atención.'],
        ...
        experienceText: 'El producto prioriza información esencial: solicitud, coordinación, seguimiento y continuidad del cuidado. Las visualizaciones muestran la dirección de la experiencia digital de Nvet.',
        ...
        vetText: 'Herramientas para gestionar disponibilidad, atención a domicilio y relación con cada paciente.',
        ...
        coverageEyebrow: 'Cobertura inicial',
        ...
        coverageSub: 'La cobertura activa se comunicará dentro del producto según disponibilidad real.',
        ...
        trustItems: [
          ['Identidad', 'Flujos de verificación para profesionales y cuentas de usuario.', UserCheck],
          ['Trazabilidad', 'Registro de interacciones y operaciones relevantes de la atención.', ShieldCheck],
          ['Continuidad', 'Información vinculada a la mascota para mantener el contexto entre visitas.', Heart],
          ['Privacidad', 'Acceso a información sensible sujeto al rol y al propósito del servicio.', ShieldCheck],
        ...
        faqs: [
          ['¿Nvet Care ya está disponible?', 'Nvet está en fase beta y Cartagena es su mercado inicial. La disponibilidad de funciones puede cambiar a medida que la plataforma completa su validación.'],
    ...
    : {
        ...
        heroTitle: 'The veterinarian you need, where your pet feels safest.',
        heroText: 'Nvet Care connects families with veterinary professionals to coordinate at-home care, follow each visit, and keep pet care organized in one place.',
        ...
        arrivalStatus: 'Care follow-up',
        petHistory: 'Pet history',
        petHistoryText: 'A continuous view of care and visits.',
        trustStrip: [
          ['Verified professionals', 'Identity and credentials within the Nvet flow.'],
          ['Care at home', 'An experience designed around pet wellbeing.'],
          ['Traceable payments', 'Payment and record flows within the platform.'],
          ['Organized history', 'Each visit linked to the pet profile.'],
        ],
        ...
        steps: [
          ...
          ['02', 'Connect', 'Nvet organizes the encounter with a professional suited for the visit.'],
        ...
        experienceText: 'The product prioritizes essential information: request, coordination, follow-up, and continuity of care. These visuals show the direction of Nvet’s digital experience.',
        ...
        vetText: 'Tools to manage availability, at-home care, and the relationship with each patient.',
        ...
        coverageEyebrow: 'Initial coverage',
        ...
        coverageSub: 'Active coverage will be communicated inside the product according to real availability.',
        ...
        trustItems: [
          ['Identity', 'Verification flows for professionals and user accounts.', UserCheck],
          ['Traceability', 'Records of relevant care interactions and operations.', ShieldCheck],
          ['Continuity', 'Information linked to the pet to preserve context between visits.', Heart],
          ['Privacy', 'Access to sensitive information is subject to role and service purpose.', ShieldCheck],
        ] as [string, string, LucideIcon][],
        ...
        faqs: [
          ['Is Nvet Care already available?', 'Nvet is in beta and Cartagena is its initial market. Feature availability can change while the platform completes validation.'],
  ...
  const trustIcons: LucideIcon[] = [UserCheck, Home, CreditCard, PawPrint];

  return (
    <main className="overflow-hidden bg-white text-[#0D1B2A]">
      ...
      <section className="border-y border-[#0D1B2A]/[0.05] bg-[#FBFCFC] py-8">
        <Container><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{copy.trustStrip.map(([title, text], index) => { const Icon = trustIcons[index]; return <div key={title} className="flex items-start gap-3 px-2 py-2"><Icon size={18} className="mt-0.5 shrink-0 text-[#34B27A]" aria-hidden="true" /><div><p className="text-xs font-semibold text-[#0D1B2A]">{title}</p><p className="mt-1 text-[11px] leading-5 text-[#73808B]">{text}</p></div></div>; })}</div></Container>
      </section>
      ...
      <section id="como-funciona" className="scroll-mt-24 bg-white py-20 sm:py-28">
        <Container>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {copy.steps.map(([number, title, text], index) => { const icons: LucideIcon[] = [MessageCircle, UserCheck, Navigation, CalendarCheck]; const Icon = icons[index]; return <FadeInSection key={number} delay={index * 0.04}><article className="group h-full rounded-[1.5rem] border border-[#0D1B2A]/[0.07] bg-white p-6 transition duration-300 hover:-translate-y-1 hover:border-[#34B27A]/25 hover:shadow-[0_18px_44px_rgba(13,27,42,0.08)]"><div className="mb-8 flex items-center justify-between"><span className="text-xs font-bold tracking-[0.12em] text-[#FF8A3D]">{number}</span><FeatureIcon icon={Icon} /></div><h3 className="text-lg font-semibold text-[#0D1B2A]" style={poppinsFont}>{title}</h3><p className="mt-3 text-sm leading-6 text-[#66737E]">{text}</p></article></FadeInSection>; })}
          </div>
        </Container>
      </section>
      ...
      <section className="border-y border-[#0D1B2A]/[0.05] bg-[#F6FAF8] py-20 sm:py-28">
        <Container>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <FadeInSection direction="left"><article className="relative overflow-hidden rounded-[2rem] bg-[#0D1B2A] p-6 text-white sm:p-8"><div className="relative z-10 max-w-sm"><span className="inline-flex rounded-full bg-white/10 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#AEE7C9]">Mobile experience</span><h3 className="mt-5 text-2xl font-semibold tracking-[-0.03em]" style={poppinsFont}>{copy.phoneCardTitle}</h3><p className="mt-3 text-sm leading-6 text-white/65">{copy.phoneCardText}</p></div><div className="mt-8 flex justify-center sm:justify-end"><Image src="/images/nvetcareapp/home-screen-phone.jpg" alt={copy.phoneCardTitle} width={1122} height={1402} sizes="(min-width:1024px) 430px, 90vw" className="h-[410px] w-auto rounded-[1.5rem] object-cover object-top shadow-[0_22px_50px_rgba(0,0,0,0.25)]" /></div></article></FadeInSection>
            <FadeInSection direction="right" delay={0.05}><article className="relative overflow-hidden rounded-[2rem] border border-[#0D1B2A]/[0.07] bg-white p-6 sm:p-8"><div className="max-w-sm"><span className="inline-flex rounded-full bg-[#34B27A]/[0.08] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#23865A]">Visit tracking</span><h3 className="mt-5 text-2xl font-semibold tracking-[-0.03em] text-[#0D1B2A]" style={poppinsFont}>{copy.trackingCardTitle}</h3><p className="mt-3 text-sm leading-6 text-[#66737E]">{copy.trackingCardText}</p></div><div className="mt-8 flex justify-center"><Image src={es ? '/images/nvetcareapp/vet-tracking-mockup-es.png' : '/images/nvetcareapp/vet-tracking-mockup-en.png'} alt={copy.trackingCardTitle} width={432} height={348} sizes="(min-width:1024px) 390px, 85vw" className="h-auto w-full max-w-[390px] drop-shadow-[0_22px_42px_rgba(13,27,42,0.16)]" /></div></article></FadeInSection>
          </div>
        </Container>
      </section>
      ...
      <section className="bg-white py-20 sm:py-28">
        <Container>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <FadeInSection direction="left"><article id="familias" className="scroll-mt-24 overflow-hidden rounded-[2rem] border border-[#0D1B2A]/[0.07] bg-white shadow-[0_16px_48px_rgba(13,27,42,0.06)]"><div className="relative h-56 sm:h-72"><Image src="/images/nvetcareapp/owner-and-dog.jpg" alt={copy.ownerTitle} fill sizes="(min-width:1024px) 50vw, 100vw" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#0D1B2A]/50 via-transparent to-transparent" /></div><div className="p-7 sm:p-8"><FeatureIcon icon={PawPrint} /><h3 className="mt-6 text-2xl font-semibold tracking-[-0.03em] text-[#0D1B2A]" style={poppinsFont}>{copy.ownerTitle}</h3><p className="mt-3 text-sm leading-6 text-[#66737E]">{copy.ownerText}</p><ul className="mt-6 grid gap-3 sm:grid-cols-2">{copy.ownerItems.map((item) => <li key={item} className="flex items-start gap-2 text-xs leading-5 text-[#4E5B66]"><Check size={15} className="mt-0.5 shrink-0 text-[#34B27A]" />{item}</li>)}</ul></div></article></FadeInSection>
            <FadeInSection direction="right" delay={0.05}><article id="veterinarios" className="scroll-mt-24 h-full rounded-[2rem] bg-[#0D1B2A] p-7 text-white sm:p-9"><div className="flex h-full flex-col"><FeatureIcon icon={Stethoscope} /><h3 className="mt-6 text-2xl font-semibold tracking-[-0.03em]" style={poppinsFont}>{copy.vetTitle}</h3><p className="mt-3 max-w-lg text-sm leading-6 text-white/65">{copy.vetText}</p><ul className="mt-7 space-y-4">{copy.vetItems.map((item) => <li key={item} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white/85"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#34B27A]/15"><Check size={13} className="text-[#6AD49C]" /></span>{item}</li>)}</ul><a href="#confianza" className="mt-9 inline-flex w-fit items-center gap-2 text-xs font-semibold uppercase tracking-[0.1em] text-[#7FE0AC] transition hover:text-white">{copy.vetCta} <ArrowRight size={14} /></a></div></article></FadeInSection>
          </div>
        </Container>
      </section>

      <section className="bg-[#F6FAF8] py-20 sm:py-28">
        <Container><div className="grid items-center gap-10 lg:grid-cols-[0.9fr_1.1fr]"><FadeInSection><div><Eyebrow>{copy.coverageEyebrow}</Eyebrow><h2 className="mt-5 text-3xl font-bold tracking-[-0.035em] text-[#0D1B2A] sm:text-4xl" style={poppinsFont}>{copy.coverageTitle}</h2><p className="mt-5 max-w-xl text-sm leading-7 text-[#5B6670] sm:text-base">{copy.coverageText}</p></div></FadeInSection><FadeInSection direction="left" delay={0.05}><div className="relative min-h-[360px] overflow-hidden rounded-[2rem] border border-[#0D1B2A]/[0.07] bg-[#DFF3E8] shadow-[0_18px_48px_rgba(13,27,42,0.08)]"><div className="absolute inset-0 opacity-60" style={{ backgroundImage: 'radial-gradient(circle at 18% 22%, rgba(52,178,122,.22) 0 2px, transparent 3px), radial-gradient(circle at 72% 38%, rgba(13,27,42,.08) 0 2px, transparent 3px)', backgroundSize: '48px 48px, 64px 64px' }} aria-hidden="true" /><div className="absolute -left-20 bottom-[-7rem] h-80 w-80 rotate-12 rounded-[45%] border-[30px] border-white/55" aria-hidden="true" /><div className="absolute -right-16 top-[-8rem] h-80 w-80 -rotate-12 rounded-[48%] border-[34px] border-white/55" aria-hidden="true" /><div className="relative flex min-h-[360px] items-center justify-center p-8 text-center"><div className="max-w-sm rounded-[1.7rem] border border-white/70 bg-white/90 p-7 shadow-[0_20px_50px_rgba(13,27,42,0.12)] backdrop-blur"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-[#34B27A] text-white shadow-[0_10px_28px_rgba(52,178,122,0.28)]"><MapPin size={21} /></span><p className="mt-5 text-base font-semibold text-[#0D1B2A]" style={poppinsFont}>{copy.coverageLabel}</p><p className="mt-3 text-xs leading-6 text-[#66737E]">{copy.coverageSub}</p></div></div></div></FadeInSection></div></Container>
      </section>

      <section id="confianza" className="scroll-mt-24 bg-white py-20 sm:py-28">
        <Container><SectionHeading eyebrow={copy.trustEyebrow} title={copy.trustTitle} text={copy.trustText} /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{copy.trustItems.map(([title, text, Icon], index) => <FadeInSection key={title} delay={index * 0.04}><article className="h-full rounded-[1.5rem] border border-[#0D1B2A]/[0.07] bg-white p-6"><FeatureIcon icon={Icon} /><h3 className="mt-6 text-base font-semibold text-[#0D1B2A]" style={poppinsFont}>{title}</h3><p className="mt-3 text-xs leading-6 text-[#66737E]">{text}</p></article></FadeInSection>)}</div></Container>
      </section>

      <section className="bg-[#0D1B2A] py-20 text-white sm:py-24">
        <Container><FadeInSection><div className="grid items-center gap-8 lg:grid-cols-[1.08fr_0.92fr]"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#72DBA2]">Nvet Care</p><h2 className="mt-4 max-w-2xl text-3xl font-bold tracking-[-0.035em] sm:text-4xl" style={poppinsFont}>{copy.missionTitle}</h2><p className="mt-5 max-w-2xl text-sm leading-7 text-white/65 sm:text-base">{copy.missionText}</p></div><div className="relative h-64 overflow-hidden rounded-[1.7rem] border border-white/10 sm:h-72"><Image src="/images/nvetcareapp/mission-banner.jpg" alt={copy.missionTitle} fill sizes="(min-width:1024px) 45vw, 100vw" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#0D1B2A]/45 to-transparent" /></div></div></FadeInSection></Container>
      </section>

      <section className="bg-white py-20 sm:py-28">
        <Container><div className="mx-auto max-w-3xl"><h2 className="text-center text-3xl font-bold tracking-[-0.035em] text-[#0D1B2A] sm:text-4xl" style={poppinsFont}>{copy.faqTitle}</h2><div className="mt-10 divide-y divide-[#0D1B2A]/[0.07] border-y border-[#0D1B2A]/[0.07]">{copy.faqs.map(([question, answer]) => <div key={question} className="py-6"><h3 className="text-sm font-semibold text-[#0D1B2A]" style={poppinsFont}>{question}</h3><p className="mt-3 text-sm leading-7 text-[#66737E]">{answer}</p></div>)}</div></div></Container>
      </section>

      <section className="bg-[#F4FBF7] py-16 sm:py-20">
        <Container><div className="mx-auto max-w-4xl rounded-[2rem] border border-[#34B27A]/15 bg-white px-6 py-10 text-center shadow-[0_20px_60px_rgba(13,27,42,0.07)] sm:px-10 sm:py-12"><Smartphone size={26} className="mx-auto text-[#34B27A]" aria-hidden="true" /><h2 className="mx-auto mt-5 max-w-2xl text-3xl font-bold tracking-[-0.035em] text-[#0D1B2A]" style={poppinsFont}>{copy.finalTitle}</h2><p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[#66737E]">{copy.finalText}</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><Link href="/nvetcareapp/iniciar-sesion" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#34B27A] px-6 text-xs font-semibold uppercase tracking-[0.08em] text-white transition hover:bg-[#2D9F6D]">{copy.finalPrimary} <ArrowRight size={14} /></Link><Link href="/changelog" className="inline-flex min-h-12 items-center justify-center rounded-full border border-[#0D1B2A]/10 bg-white px-6 text-xs font-semibold uppercase tracking-[0.08em] text-[#0D1B2A] transition hover:bg-[#F7F9F8]">{copy.finalSecondary}</Link></div></div></Container>
      </section>
    </main>
  );
};
