...
import { ArrowUpRight, Boxes, ChartNoAxesCombined, ScanLine } from 'lucide-react';
...
export const InvestmentSpotlightSection: React.FC = () => {
  ...
  return (
    <section
      className="relative overflow-hidden border-y border-white/[0.045] py-20 sm:py-24 md:py-28 lg:py-32"
      ...
      <Container className="relative z-10">
        <div className="grid items-stretch gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:gap-14 xl:gap-20">
          <FadeInSection direction="left">
            <div className="relative min-h-[420px] overflow-hidden border border-white/[0.065] bg-black sm:min-h-[500px] lg:min-h-[620px]">
              <Image
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
                <div className="inline-flex items-center gap-2 border border-accent/20 bg-black/70 px-3 py-2 backdrop-blur-md">
                  <span className="h-1.5 w-1.5 rounded-full bg-accent" />
                  <span className="text-[9px] uppercase tracking-[0.22em] text-accent">CTG Craft Beer · Cartagena</span>
                </div>
              </div>
            </div>
          </FadeInSection>
          ...
          <FadeInSection direction="right" delay={0.08}>
            <div className="flex h-full flex-col justify-center py-2 lg:py-8">
              <div className="mb-5 flex items-center gap-3">
                ...
                <span className="text-[9px] uppercase tracking-[0.25em] text-accent sm:text-[10px]">{copy.eyebrow}</span>
              </div>
              ...
              <div className="mb-9 grid gap-3 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
                {copy.signals.map(({ icon: Icon, label, detail }) => (
                  <div
                    key={label}
                    className="flex min-w-0 items-start gap-3 border border-white/[0.055] bg-white/[0.012] p-4"
                  >
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border border-accent/25 bg-accent/[0.035]">
                      <Icon className="h-4 w-4 shrink-0 text-accent" strokeWidth={1.45} />
                    </span>
                    <div className="min-w-0 pt-0.5">
                      <span className="block text-[10px] font-medium uppercase leading-tight tracking-[0.13em] text-white">{label}</span>
                      <span className="mt-1.5 block text-[11px] leading-relaxed text-text-dim">{detail}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
                ...
                <a
                  href="/inversion"
                  className="group inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-text-dim transition-colors duration-300 hover:text-accent"
                >
                  ctgone.com/inversion
