...
const ITEMS = [
  ...
  { href: '/token', badge: TOKEN.badge, title: TOKEN.title, titleHighlight: TOKEN.titleHighlight, description: TOKEN.description, icon: Wallet },
...
export const HomeOverviewSection: React.FC = () => {
  ...
  const intro = locale === 'es'
    ? {
        ...
        description: 'Cada unidad de CTG One funciona como un entorno operativo donde nuestro software, automatización e infraestructura se prueban, despliegan y mejoran continuamente.',
      }
    : {
        eyebrow: 'Ecosystem architecture',
        title: 'One technology layer.',
        highlight: 'Multiple real operations.',
        description: 'Every CTG One unit acts as an operating environment where our software, automation, and infrastructure are tested, deployed, and continuously improved.',
      };

  return (
    <section
      className="relative py-24 sm:py-32 md:py-36 lg:py-44 overflow-hidden"
      style={{ backgroundColor: 'var(--bg-secondary)' }}
    >
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <div
          className="absolute left-1/2 top-[7%] -translate-x-1/2 w-[760px] h-[760px] rounded-full"
          style={{
            background: 'radial-gradient(circle, rgba(212,162,89,0.045) 0%, rgba(212,162,89,0.012) 38%, transparent 70%)',
          }}
        />
        <div
          ...
          <div className="grid lg:grid-cols-[0.78fr_1.22fr] gap-8 lg:gap-16 items-end mb-14 sm:mb-18 md:mb-20">
            <div>
              <div className="inline-flex items-center gap-2.5 mb-5">
                ...
                <span className="text-[9px] sm:text-[10px] uppercase tracking-[0.25em] text-accent">
                  {intro.eyebrow}
                </span>
              </div>
          ...
          {ITEMS.map((item, index) => {
            ...
            return (
              <FadeInSection key={item.href} delay={0.05 + index * 0.055}>
                <a
                  href={item.href}
                  className="group relative block min-h-[320px] sm:min-h-[345px] p-7 sm:p-8 overflow-hidden border border-white/[0.055] bg-[#080808]/90 transition-all duration-500 hover:border-accent/35 hover:-translate-y-1"
                  style={{ boxShadow: '0 18px 70px rgba(0,0,0,0.18)' }}
                >
                  <div
                    className="absolute -right-24 -top-24 w-64 h-64 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700"
                    style={{ background: 'radial-gradient(circle, rgba(212,162,89,0.13), transparent 67%)' }}
                  />
                  ...
                  <div className="relative z-10 h-full flex flex-col">
                    <div className="flex items-start justify-between gap-5 mb-10">
                      ...
                      <span className="font-outfit text-[10px] tracking-[0.22em] text-text-dim group-hover:text-accent/70 transition-colors duration-500">
                        {number}
                      </span>
                    </div>

                    <span className="block text-[9px] uppercase tracking-[0.2em] text-accent/80 mb-3">
                      {t(item.badge)}
                    </span>
                    <h3 className="text-xl sm:text-2xl font-outfit font-medium tracking-[-0.02em] leading-tight mb-4">
                      ...
                      <span className="text-text-muted group-hover:text-accent transition-colors duration-500">{t(item.titleHighlight)}</span>
                    </h3>
                    <p className="text-[13px] sm:text-sm text-text-muted leading-relaxed mb-8 line-clamp-4">
                      {t(item.description)}
                    </p>

                    <div className="mt-auto flex items-center justify-between border-t border-white/[0.045] pt-5">
                      <span className="text-[10px] uppercase tracking-[0.18em] text-text-dim group-hover:text-accent transition-colors duration-500">
                        {t('See more')}
                      </span>
