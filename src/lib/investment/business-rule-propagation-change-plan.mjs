...
const ACTIONS = new Set([
  'MODIFY_EXISTING',
  'CREATE_OR_MODIFY',
  'NEW_IMMUTABLE_MIGRATION',
  'EXTEND_CONTRACT_TESTS',
  'EXTEND_OPERATOR_TOOLING',
]);
const TARGET_KINDS = new Set(['FILE', 'DIRECTORY']);
...
export function validateInvestmentBusinessRulePropagationChangeBlueprint(blueprint) {
  ...
  assert(Array.isArray(blueprint.stages) && blueprint.stages.length === INVESTMENT_BUSINESS_RULE_PROPAGATION_STAGE_IDS.length, 'Propagation change blueprint must contain exactly four stages');
  const stageIds = new Set();
  let priorOrder = -Infinity;
  for (const stage of blueprint.stages) {
    assert(INVESTMENT_BUSINESS_RULE_PROPAGATION_STAGE_IDS.includes(stage.id), `Unknown propagation change stage: ${stage.id}`);
    assert(!stageIds.has(stage.id), `Duplicate propagation change stage: ${stage.id}`);
    stageIds.add(stage.id);
    assert(Number.isInteger(stage.order) && stage.order > priorOrder, `Propagation change stage ${stage.id} order must be strictly increasing`);
    priorOrder = stage.order;
  ...
  assert(Array.isArray(blueprint.surfaces) && blueprint.surfaces.length === INVESTMENT_BUSINESS_RULE_PROPAGATION_SURFACE_IDS.length, 'Propagation change blueprint must contain exactly seven authority surfaces');
  ...
  for (const surface of blueprint.surfaces) {
    assert(INVESTMENT_BUSINESS_RULE_PROPAGATION_SURFACE_IDS.includes(surface.id), `Unknown propagation change surface: ${surface.id}`);
    assert(!surfaceIds.has(surface.id), `Duplicate propagation change surface: ${surface.id}`);
    ...
    for (const task of surface.tasks) {
      assert(typeof task.id === 'string' && /^[a-z0-9][a-z0-9-]{2,119}$/.test(task.id), `${surface.id} contains an invalid task id`);
      assert(!taskIds.has(task.id), `Duplicate propagation change task id: ${task.id}`);
      taskIds.add(task.id);
      assert(ACTIONS.has(task.action), `${task.id} has invalid action: ${task.action}`);
      assert(Array.isArray(task.ruleIds) && task.ruleIds.length > 0, `${task.id} must reference at least one BR`);
      const seenRules = new Set();
      for (const ruleId of task.ruleIds) {
        assert(INVESTMENT_REQUIRED_BUSINESS_RULE_IDS.includes(ruleId), `${task.id} references unknown business rule: ${ruleId}`);
        assert(!seenRules.has(ruleId), `${task.id} contains duplicate business rule: ${ruleId}`);
        seenRules.add(ruleId);
        coverage.get(ruleId).add(surface.id);
      }
      assert(Array.isArray(task.targets) && task.targets.length > 0, `${task.id} requires at least one target`);
      const targetKeys = new Set();
      for (const target of task.targets) {
        assert(TARGET_KINDS.has(target.kind), `${task.id} has invalid target kind: ${target.kind}`);
        validateRepositoryPath(target.path, `${task.id}.target`);
        const key = `${target.kind}:${target.path}`;
        assert(!targetKeys.has(key), `${task.id} contains duplicate target: ${key}`);
        targetKeys.add(key);
      }
  ...
  for (const requiredSurface of INVESTMENT_BUSINESS_RULE_PROPAGATION_SURFACE_IDS) {
    assert(surfaceIds.has(requiredSurface), `Missing propagation change surface: ${requiredSurface}`);
  }
...
export function buildInvestmentBusinessRulePropagationChangePlan({
  ...
  return deepFreeze({
    ...
    requiredTaskCount: blueprint.surfaces.reduce((sum, surface) => sum + surface.tasks.length, 0),
    blueprint,
  });
}
