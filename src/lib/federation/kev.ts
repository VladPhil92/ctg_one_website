...
export const KEV_CANONICAL_ORIGIN = 'https://kev.ctgone.com';
export const KEV_FEDERATION_PROVIDER = 'kev' as const;
export const KEV_AUTHORIZATION_CODE_TTL_MS = 60_000;
...
export function federationSecretState(
  request: Request,
): 'unconfigured' | 'authorized' | 'unauthorized' {
  const expected = process.env.KEV_FEDERATION_SECRET?.trim() ?? '';
