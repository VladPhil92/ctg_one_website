...
const STEPS = [
  { n: '01', title: 'Explora oportunidades', description: 'Revisa los lotes publicados, sus estilos, cantidades, estado y condiciones.', icon: Search },
  { n: '02', title: 'Participa en un lote', description: 'Cuando un lote abre financiación, elige cuánto participar y confirma tu aporte de forma segura.', icon: Coins },
  { n: '03', title: 'Sigue la producción', description: 'Observa el avance del lote desde la elaboración hasta el envasado.', icon: Factory },
  { n: '04', title: 'Sigue las ventas', description: 'Consulta la comercialización en puntos propios y canal B2B.', icon: BarChart3 },
  { n: '05', title: 'Consulta la liquidación', description: 'Revisa ingresos, costos, impuestos y resultado final del lote.', icon: FileText },
  { n: '06', title: 'Retira o reinvierte', description: 'Decide qué hacer con tu saldo disponible al finalizar el proceso.', icon: RefreshCw },
];
...
const BEER_STYLES = [
  {
    ...
    focusClass: publicStyles.focusGolden,
  },
  {
    name: 'Hefeweizen',
    ...
    focusClass: publicStyles.focusHefeweizen,
  },
  {
    name: 'Porter',
    origin: 'CTG Craft Beer',
    ...
    focusClass: publicStyles.focusPorter,
  },
  {
    name: 'Irish Red Ale',
    origin: 'CTG Craft Beer',
    ...
    focusClass: publicStyles.focusIrish,
  },
];
    ...
    <>
          ...
          <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-12 lg:gap-16 items-center">
            <FadeInSection>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Button href="/inversion/lotes" variant="primary" size="lg" arrow>Ver lotes publicados</Button>
                <Button href="/inversion/como-funciona" variant="secondary" size="lg">Cómo funciona</Button>
              </div>
            </FadeInSection>

            <FadeInSection delay={0.12}>
              <div className={`${publicStyles.mediaFrame} ${publicStyles.mediaHero} relative max-w-xl mx-auto lg:ml-auto w-full`}>
                <Image
                  ...
                  priority
                  quality={90}
                  sizes="(max-width: 640px) 94vw, (max-width: 1024px) 86vw, 42vw"
                  className={`${publicStyles.mediaImage} ${publicStyles.focusMiyagi}`}
                  data-ctg-photo="high-fidelity-source"
                />
                <div className="absolute inset-x-0 bottom-0 z-[3] bg-gradient-to-t from-black/90 via-black/50 to-transparent px-6 pb-6 pt-20">
            ...
            {BEER_STYLES.map((beer, index) => (
              <FadeInSection key={beer.name} delay={0.04 + index * 0.06}>
                <article className="group overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.025] h-full">
                  <div className={`${publicStyles.mediaFrame} ${publicStyles.mediaCard} relative w-full bg-black`}>
                    <Image
                      src={beer.image}
                      alt={beer.alt}
                      fill
                      ...
                      className={`${publicStyles.mediaImage} ${beer.focusClass}`}
                      data-ctg-photo="high-fidelity-source"
                    />
                    <div className="absolute inset-0 z-[1] bg-gradient-to-t from-black/72 via-transparent to-transparent" />
                  </div>
              ...
              return (
                <FadeInSection key={step.n} delay={i * 0.05}>
                  <article className="group relative h-full overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.025] p-6 sm:p-7 transition-all duration-300 hover:-translate-y-1 hover:border-[rgba(201,169,98,0.28)] hover:bg-white/[0.04]">
                    <div className="absolute -right-16 -top-16 h-40 w-40 rounded-full opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-100" style={{ backgroundColor: 'rgba(201, 169, 98, 0.09)' }} aria-hidden="true" />
                    <div className="relative flex items-start justify-between gap-5 mb-8">
                      <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full border border-[rgba(201,169,98,0.22)]" style={{ background: 'radial-gradient(circle at 50% 45%, rgba(201,169,98,0.15), rgba(201,169,98,0.035) 70%)', boxShadow: '0 0 34px rgba(201,169,98,0.07)' }}>
                        <Icon aria-hidden="true" className="h-7 w-7 text-accent" strokeWidth={1.55} />
                      </div>
                      <span className="font-outfit text-3xl font-semibold tracking-tight text-accent/90">{step.n}</span>
                    </div>
                    <div className="relative">
                      ...
                      <h3 className="mb-2 text-lg font-outfit font-medium text-white">{step.title}</h3>
        ...
        <Container>
          <FadeInSection>
            <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
              <SectionHeader badge="Oportunidades" title="Lotes de" titleHighlight="producción" centered={false} className="max-w-lg" />
              <Button href="/inversion/lotes" variant="ghost" size="sm" arrow>Ver todos los lotes</Button>
            </div>
          </FadeInSection>
          {lots.length > 0 ? (
            ...
            <p className="text-sm text-text-dim">Aún no hay lotes publicados. Los borradores internos no se muestran en esta superficie.</p>
          )}
        </Container>
      </section>
      ...
      <section className="py-20 sm:py-24">
        <Container size="small">
          <FadeInSection>
            <div className="p-8 sm:p-12 rounded-lg text-center" style={{ backgroundColor: 'rgba(201, 169, 98, 0.05)', border: '1px solid var(--border-accent, rgba(201,169,98,0.2))' }}>
              <h2 className="text-xl sm:text-2xl font-outfit font-semibold text-white mb-3">¿Quieres estimar una participación?</h2>
              <p className="text-sm text-text-muted max-w-md mx-auto mb-8 leading-relaxed">
                Usa el simulador para ver, con cifras ilustrativas, cómo se comportaría tu participación en un lote. No constituye una rentabilidad garantizada.
              </p>
              <Button href="/inversion/simulador" variant="primary" size="md" arrow>Abrir simulador</Button>
            </div>
          </FadeInSection>
        </Container>
      </section>
