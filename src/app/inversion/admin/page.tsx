...
// Admin dashboard — real data. Interim gate: reuses the existing CTG One
// `profiles.role === 'admin'` flag (ADR-011) at the page/middleware level;
// every mutation below is re-authorized server-side against
// investment_participant_profiles.investment_role regardless of that gate.
export default function InvestmentAdminPage() {
  ...
  const [form, setForm] = useState({ code: '', beerStyle: '', destination: 'Bogotá', totalCases: '50', caseSizeUnits: '24' });
  ...
  const handleCreateLot = async () => {
    ...
    try {
      ...
      setForm({ code: '', beerStyle: '', destination: 'Bogotá', totalCases: '50', caseSizeUnits: '24' });
      await refresh();
    } catch (err) {
  ...
  return (
    <section className="py-12 sm:py-16">
      <Container>
        <h1 className="text-2xl sm:text-3xl font-outfit font-semibold text-white mb-10">
        ...
        <Card variant="bordered" padding="md" className="mb-12">
          <p className="text-sm font-medium text-white mb-4">Crear lote de producción</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-3">
            <input placeholder="Código (PB-BOG-...)" value={form.code}
              onChange={(e) => setForm({ ...form, code: e.target.value })}
              className="px-3 py-2 rounded text-sm text-white" style={{ backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border)' }} />
            <input placeholder="Estilo de cerveza" value={form.beerStyle}
              onChange={(e) => setForm({ ...form, beerStyle: e.target.value })}
              className="px-3 py-2 rounded text-sm text-white" style={{ backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border)' }} />
            <input placeholder="Destino" value={form.destination}
              onChange={(e) => setForm({ ...form, destination: e.target.value })}
              className="px-3 py-2 rounded text-sm text-white" style={{ backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border)' }} />
            <input type="number" placeholder="Total cajas" value={form.totalCases}
              onChange={(e) => setForm({ ...form, totalCases: e.target.value })}
              className="px-3 py-2 rounded text-sm text-white" style={{ backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border)' }} />
            <input type="number" placeholder="Botellas/caja" value={form.caseSizeUnits}
              onChange={(e) => setForm({ ...form, caseSizeUnits: e.target.value })}
              className="px-3 py-2 rounded text-sm text-white" style={{ backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border)' }} />
          </div>
          {formError && <p className="text-[12px] mb-2" style={{ color: 'var(--error)' }}>{formError}</p>}
          <Button onClick={handleCreateLot} loading={isSubmitting} variant="primary" size="sm">
            Crear lote
          </Button>
        </Card>
          ...
          {summary.lots.map((lot) => {
            ...
            return (
              <Card key={lot.id} variant="bordered" padding="md">
                ...
                {nextStatus && (
                  <Button
                    onClick={() => handleAdvance(lot.id, nextStatus)}
                    loading={transitioningId === lot.id}
                    variant="secondary"
                    size="sm"
                  >
