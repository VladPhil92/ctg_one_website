  ...
  return (
    <div className="investment-console min-h-screen bg-[#050505] text-white overflow-hidden">
      ...
      <section className="relative py-14 sm:py-20">
        <Container>
          <a href="/dashboard" className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[.18em] text-text-dim hover:text-accent transition-colors mb-5">
            <ArrowLeft size={13} /> User OS
          </a>

          <div className="investment-panel rounded-[28px] p-6 sm:p-9 mb-8 relative overflow-hidden">
            <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-accent/50 to-transparent" />
            <div className="absolute right-[-5rem] top-[-6rem] w-72 h-72 rounded-full border border-accent/10" />
            <div className="absolute right-[-1rem] top-[-2rem] w-40 h-40 rounded-full border border-accent/15" />
            <div className="relative grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-7 items-end">
              <div>
                ...
                <p className="text-sm sm:text-base text-text-muted mt-5 max-w-2xl leading-relaxed">Selecciona oportunidades, registra tu participación y sigue el movimiento real de cada lote desde financiación hasta liquidación.</p>
              </div>
          ...
          {ordersLoading ? (
            <div className="investment-panel rounded-2xl p-7"><p className="text-sm text-text-dim">Sincronizando inversiones...</p></div>
          ) : orders.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              {orders.map((order) => (
                <div key={order.id} className="investment-panel investment-card rounded-[22px] p-5 sm:p-6 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    {order.status === 'ALLOCATED' && order.lot ? (
                      ...
                      <div className="rounded-xl border border-white/[.07] p-4 bg-white/[.015]">
