...
export default function EducationServiceOperationsPage() {
  ...
  async function updateSessionStatus(sessionId: string, status: 'completed' | 'cancelled' | 'no_show') {
    const labels = { completed: 'Sesión marcada como completada.', cancelled: 'Sesión cancelada.', no_show: 'Inasistencia registrada.' } as const;
    const ok = await send({ action: 'update_session_status', requestId: selectedId, sessionId, status });
    if (ok) setNotice(labels[status]);
  }
  ...
  return <div className="min-h-screen bg-[#050505] text-white"><Navbar /><main className="pb-20 pt-24"><Container>
    ...
    <section className="mt-8 grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
      ...
      <div className="space-y-5">{selected ? <>
        ...
        <div className="grid gap-4 lg:grid-cols-2"><article className="rounded-2xl border border-white/10 bg-white/[.025] p-5"><h3 className="font-outfit text-lg font-semibold">Propuestas</h3><div className="mt-4 space-y-3">{quotes.length === 0 ? <p className="text-xs text-white/35">Sin propuestas.</p> : quotes.map((quote) => <div key={quote.id} className="rounded-xl border border-white/[.07] bg-black/20 p-4"><div className="flex justify-between gap-3"><p className="text-sm font-semibold">v{quote.version} · {amount(quote.total_amount, quote.currency)}</p><span className="text-[9px] uppercase text-accent">{quote.status}</span></div><p className="mt-2 text-xs text-white/40">{quote.title}</p></div>)}</div></article><article className="rounded-2xl border border-white/10 bg-white/[.025] p-5"><h3 className="font-outfit text-lg font-semibold">Agenda y cumplimiento</h3><div className="mt-4 space-y-3">{sessions.length === 0 ? <p className="text-xs text-white/35">Sin sesiones.</p> : sessions.map((session) => <div key={session.id} className="rounded-xl border border-white/[.07] bg-black/20 p-4"><div className="flex justify-between gap-3"><p className="text-sm font-semibold">{session.title}</p><span className="text-[9px] uppercase text-accent">{session.status}</span></div><p className="mt-2 text-xs text-white/40">{dt(session.starts_at)} · {session.modality}</p>{session.status === 'scheduled' ? <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={submitting} onClick={() => void updateSessionStatus(session.id, 'completed')} className="min-h-9 rounded-lg border border-emerald-400/20 bg-emerald-500/10 px-3 text-[9px] font-bold uppercase tracking-[.1em] text-emerald-200 disabled:opacity-40">Completar</button><button type="button" disabled={submitting} onClick={() => void updateSessionStatus(session.id, 'no_show')} className="min-h-9 rounded-lg border border-amber-400/20 bg-amber-500/10 px-3 text-[9px] font-bold uppercase tracking-[.1em] text-amber-200 disabled:opacity-40">No asistió</button><button type="button" disabled={submitting} onClick={() => void updateSessionStatus(session.id, 'cancelled')} className="min-h-9 rounded-lg border border-red-400/20 bg-red-500/10 px-3 text-[9px] font-bold uppercase tracking-[.1em] text-red-200 disabled:opacity-40">Cancelar</button></div> : null}</div>)}</div></article></div>
