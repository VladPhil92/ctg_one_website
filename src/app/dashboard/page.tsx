...
import { Activity, ArrowUpRight, Beer, BrainCircuit, CircleDollarSign, Factory, Gift, Landmark, LockKeyhole, Radar, ShieldCheck, Sparkles, UserRound, WalletCards, Zap } from 'lucide-react';
  ...
  return <div className="min-h-screen bg-[#050505] text-white overflow-hidden"><Navbar />
    ...
    <main className="relative pt-24 pb-20"><Container>
      ...
      <section><div className="flex items-end justify-between mb-5"><div><p className="micro">ECOSYSTEM GRID</p><h2 className="text-2xl sm:text-3xl font-outfit font-semibold mt-2 tracking-[-.025em]">Módulos conectados</h2></div><Sparkles size={19} className="text-accent hidden sm:block"/></div><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {profile?.role==='admin'&&<Module icon={<Factory/>} code="ADM-00" status="ADMIN" title="CTG One Admin OS" description="Controla operaciones, producción, trazabilidad, órdenes, usuarios y conocimiento desde la capa administrativa protegida." href="/admin" cta="Abrir Admin OS" featured />}
        <Module icon={<Beer/>} code="INV-01" status="LIVE" title="Craft Beer Investment" description="Selecciona lotes y cajas, registra pagos y sigue producción, comercialización y liquidación en tiempo real." href="/dashboard/inversion" cta="Abrir terminal de inversión" featured={profile?.role!=='admin'} />
