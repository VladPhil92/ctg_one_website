...
export default function LabelsPage(){
  ...
  const qrUrl=(serial:string)=>`https://api.qrserver.com/v1/create-qr-code/?size=180x180&margin=4&data=${encodeURIComponent(traceUrl(serial))}`;
  ...
  return <div className="space-y-6">
    <header className="no-print rounded-[28px] border border-white/10 p-6 sm:p-8" style={{background:'linear-gradient(135deg,rgba(18,18,18,.98),rgba(7,7,7,.95))'}}>
      ...
      <p className="text-sm text-text-muted mt-3 max-w-3xl leading-relaxed">Genera manifiestos y hojas imprimibles a partir de botellas ya serializadas. El QR expone únicamente la URL pública de autenticidad de la unidad.</p>
    </header>
    ...
    {lot&&visible.length>0?<section className="label-sheet">
      {visible.map(b=><article key={b.id} className="beer-label">
        ...
        <div className="qr-frame"><img src={qrUrl(b.serial_code)} alt={`QR ${b.serial_code}`} loading="lazy"/></div>
