...
import { deriveLotMetrics, getStyleDefaults, hasCompleteStyleEconomics, lotCodePreview } from '@/lib/production/lot-config';
...
export default function OperationsAdminPage() {
  ...
  const refreshStyles = useCallback(async () => {
    ...
    const { data, error: styleError } = await supabase
      ...
      .select('id,code,slug,name,description,abv_target,units_per_case,standard_production_cost_unit_cents,standard_label_cost_unit_cents,standard_own_point_price_unit_cents,standard_b2b_price_unit_cents,standard_inc_rate,standard_advertising_rate_on_pre_inc,active')
      .eq('active', true)
      .order('name');
    if (styleError) {
  ...
  const run = async (fn: () => Promise<{ error?: { message:string } | null }>, success:string) => {
    ...
    try {
      ...
      setMessage(success);
      await Promise.all([refresh(), refreshStyles()]);
  ...
  return <div className="space-y-8">
    ...
    <section className="grid grid-cols-1 xl:grid-cols-[.8fr_1.2fr] gap-5">
      <CreateLotPanel
        ...
        busy={busy}
        onCreate={(payload)=>run(async()=>supabase.rpc('create_production_lot_from_style', payload),'Lote creado correctamente con snapshot económico persistido.')}
        onSaveDefaults={(payload)=>run(async()=>supabase.rpc('update_investment_beer_style_economics', payload),'Presets económicos del estilo actualizados.')}
      />
...
function CreateLotPanel({styles,loading,busy,onCreate,onSaveDefaults}:{styles:InvestmentBeerStyle[];loading:boolean;busy:boolean;onCreate:(p:Record<string,unknown>)=>void;onSaveDefaults:(p:Record<string,unknown>)=>void}){
  const [f,setF]=useState<LotEconomicsForm>({styleCode:'',destination:'Cartagena',cases:'10',caseSize:'',production:'',label:'',ownPrice:'',b2bPrice:'',inc:'',advertising:''});
  const style = styles.find(s=>s.code===f.styleCode) ?? styles[0] ?? null;

  useEffect(()=>{
    if (!styles.length) return;
    setF(current => current.styleCode ? current : draftFromStyle(styles[0], current));
  },[styles]);

  const selectStyle=(code:string)=>{
    const next=styles.find(s=>s.code===code);
    if(!next){setF({...f,styleCode:code});return;}
    setF(current=>draftFromStyle(next,current));
  };

  const cases=parsedNumber(f.cases);
  const caseSize=parsedNumber(f.caseSize);
  const production=parsedNumber(f.production);
  const label=parsedNumber(f.label);
  const ownPrice=parsedNumber(f.ownPrice);
  const b2bPrice=parsedNumber(f.b2bPrice);
  const inc=parsedNumber(f.inc);
  const advertising=parsedNumber(f.advertising);

  const economicsValid = production!=null && production>=0
  ...
  const lotValid = !!style && cases!=null && cases>0 && caseSize!=null && caseSize>0 && economicsValid && f.destination.trim().length>0;
  const metrics=deriveLotMetrics({cases:cases??0,caseSize:caseSize??0,productionCostCop:production??0,labelCostCop:label??0,ownPriceCop:ownPrice??0,b2bPriceCop:b2bPrice??0});
  ...
  const submit=(e:React.FormEvent)=>{
    ...
    onCreate({
      ...economicsPayload(),
      p_style_code:style.code,
      p_destination:f.destination.trim(),
      p_total_cases:cases,
  ...
  return <form onSubmit={submit} className="adminPanel rounded-2xl p-5 sm:p-6">
    ...
    <div className="mt-5 rounded-xl border p-4" style={{borderColor:masterConfigured?'rgba(201,169,98,.22)':'rgba(245,158,11,.24)',background:masterConfigured?'rgba(201,169,98,.035)':'rgba(245,158,11,.035)'}}>
