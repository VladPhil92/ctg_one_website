...
type Withdrawal = { id: string; participant_user_id: string; amount_cents: number; status: string; admin_notes: string | null; created_at: string };
type Destination = { user_id: string; bank_account_masked: string | null; payout_destination_fingerprint: string | null };
...
export default function PaymentRailsAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (userIds.length) {
      const { data: destinationData, error: destinationError } = await supabase.from('investment_participant_profiles').select('user_id,bank_account_masked,payout_destination_fingerprint').in('user_id', userIds);
...
function WithdrawalCard({ withdrawal, destination, rail, draft, busy, patchDraft, onApprove, onInitiate, onConfirm, onFail }: { withdrawal: Withdrawal; destination?: Destination; rail?: InvestmentPayoutReconciliation; draft: Draft; busy: boolean; patchDraft: (patch: Partial<Draft>) => void; onApprove: () => void; onInitiate: () => void; onConfirm: () => void; onFail: () => void }) {
  const destinationReady = !!destination?.bank_account_masked && !!destination?.payout_destination_fingerprint;
  return <article className="rounded-2xl border border-white/[.08] bg-white/[.02] p-5 sm:p-6">
    <div className="flex items-start justify-between gap-4 mb-5"><div><p className="text-[9px] uppercase tracking-[.16em] text-text-dim">Withdrawal</p><p className="text-xl text-white font-semibold mt-1">{formatCents(withdrawal.amount_cents)}</p><p className="text-[9px] font-mono text-text-dim mt-1">{withdrawal.id}</p></div><span className="rounded-full border border-accent/20 bg-accent/[.04] px-2.5 py-1 text-[8px] uppercase tracking-[.12em] text-accent">{withdrawal.status}</span></div>
    <div className="grid grid-cols-2 gap-3 mb-5"><Mini label="Participante" value={withdrawal.participant_user_id.slice(0, 12)} /><Mini label="Destino" value={destination?.bank_account_masked ?? 'No registrado'} alert={!destinationReady} /><Mini label="Rail state" value={rail?.payout_state ?? 'NOT_INITIATED'} /><Mini label="Ref. externa" value={rail?.external_reference ?? '—'} /></div>
    {withdrawal.status === 'REQUESTED' && <div className="border-t border-white/[.07] pt-4"><Button onClick={onApprove} disabled={!destinationReady} loading={busy} variant="primary" size="sm">Aprobar retiro</Button>{!destinationReady && <p className="text-[11px] text-amber-300/80 mt-2">El participante debe registrar primero un destino de payout.</p>}</div>}
    {withdrawal.status === 'APPROVED' && <div className="space-y-3 border-t border-white/[.07] pt-4"><div className="grid sm:grid-cols-2 gap-3"><label className="railLabel">Rail<select className="railInput mt-1.5" value={draft.payoutRail} onChange={(event) => patchDraft({ payoutRail: event.target.value as InvestmentPayoutRail })}><option value="bank_transfer">Bank transfer</option><option value="bre_b">Bre-B</option><option value="crypto">Crypto</option><option value="other">Other</option></select></label><label className="railLabel">Proveedor / banco<input className="railInput mt-1.5" value={draft.providerCode} onChange={(event) => patchDraft({ providerCode: event.target.value })} placeholder="BANCOLOMBIA" /></label></div><Button onClick={onInitiate} disabled={!destinationReady} loading={busy} variant="primary" size="sm"><Send size={13} /> Iniciar payout</Button></div>}
    {withdrawal.status === 'PAYMENT_PROCESSING' && <div className="space-y-3 border-t border-white/[.07] pt-4"><div className="grid sm:grid-cols-2 gap-3"><label className="railLabel">Referencia confirmada<input className="railInput mt-1.5" value={draft.externalReference} onChange={(event) => patchDraft({ externalReference: event.target.value })} placeholder="PAYOUT-..." /></label><label className="railLabel">Fecha/hora pagada<input type="datetime-local" className="railInput mt-1.5" value={draft.paidAt} onChange={(event) => patchDraft({ paidAt: event.target.value })} /></label></div><div className="flex flex-wrap gap-2"><Button onClick={onConfirm} loading={busy} variant="primary" size="sm"><CheckCircle2 size={13} /> Confirmar payout</Button><Button onClick={onFail} disabled={busy} variant="secondary" size="sm"><ShieldAlert size={13} /> Registrar fallo</Button></div></div>}
