...
import { Bell, CalendarDays, CheckCheck, PawPrint, ShieldCheck } from 'lucide-react';
...
export default async function NvetNotificationsPage() {
  ...
  const inboxResult = await fetchNvetNotifications(accessToken, 50);
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-7">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#5B6670]">
            Centraliza cambios reales de tus citas y alertas preventivas derivadas de las fechas que has registrado para tus mascotas.
          </p>
        </div>
        ...
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-[#34B27A]/20 bg-[#34B27A]/[0.06] p-4">
          ...
          <div>
            <p className="text-sm font-semibold text-[#0D1B2A]">Eventos verificables, no mensajes simulados</p>
            <p className="mt-1 text-xs leading-5 text-[#5B6670]">
              Las alertas preventivas provienen del expediente reportado por el cuidador y los cambios de cita provienen del ciclo operativo persistido. Esta versión es in-app; push y correo no se presentan como activos todavía.
            </p>
          </div>
        </div>
