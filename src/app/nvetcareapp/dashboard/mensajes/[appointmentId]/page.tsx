...
import { fetchNvetChatMessages, fetchNvetChatMetadata, type NvetChatAppointmentStatus } from '@/lib/nvetcareapp/chat';
...
function statusLabel(status: NvetChatAppointmentStatus) {
  const labels: Record<NvetChatAppointmentStatus, string> = {
    PENDING: 'Pendiente',
...
export default async function NvetAppointmentConversationPage({ params }: { params: Promise<{ appointmentId: string }> }) {
  ...
  const appointmentStatus = statusLabel(metadata.appointment.status);
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6">
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#34B27A]">Nvet Care · Cita {appointmentStatus.toLowerCase()}</p>
