...
import { CalendarDays, CheckCircle2, Loader2, MapPin, PawPrint, Search, Stethoscope } from 'lucide-react';
import type {
...
function vetName(vet: NvetVetSearchItem): string {
  const name = [vet.user.firstName, vet.user.lastName].filter(Boolean).join(' ').trim();
  return name || 'Veterinario Nvet Care';
}
...
export function ClientBookingFlow({ firstName }: { firstName: string }) {
  ...
  const selectedVet = useMemo(() => vets.find((vet) => vet.id === selectedVetId) ?? null, [vets, selectedVetId]);
  const selectedPrice = useMemo(
    () => prices.find((price) => price.id === selectedPriceId) ?? null,
    [prices, selectedPriceId],
  );

  async function loadVets(query = '') {
    setSearching(true);
    try {
      const params = new URLSearchParams({ limit: '20' });
      if (query.trim()) params.set('search', query.trim());
      const response = await fetch(`/api/nvetcareapp/client/vets?${params.toString()}`, { cache: 'no-store' });
      const data = await readJson<NvetVetSearchResponse & { message?: string }>(response);
      if (!response.ok || !data) throw new Error(data?.message || 'No se pudieron cargar los veterinarios.');
      setVets(Array.isArray(data.results) ? data.results : []);
    } catch (cause) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        ...
        const petsData = await readJson<NvetPet[] & { message?: string }>(petsResponse);
        const vetsData = await readJson<NvetVetSearchResponse & { message?: string }>(vetsResponse);
        if (!petsResponse.ok) throw new Error((petsData as { message?: string } | null)?.message || 'No se pudieron cargar tus mascotas.');
        if (!vetsResponse.ok || !vetsData) throw new Error(vetsData?.message || 'No se pudieron cargar los veterinarios.');
        if (!active) return;
        const petList = Array.isArray(petsData) ? petsData : [];
        setPets(petList);
        setVets(Array.isArray(vetsData.results) ? vetsData.results : []);
  ...
  async function selectVet(vetId: string) {
    ...
    try {
      const response = await fetch(`/api/nvetcareapp/client/vets/${encodeURIComponent(vetId)}/prices`, { cache: 'no-store' });
      const data = await readJson<(NvetVetPrice[] & { message?: string })>(response);
      if (!response.ok) throw new Error((data as { message?: string } | null)?.message || 'No se pudieron cargar los servicios.');
  ...
  async function selectDate(nextDate: string) {
    ...
    try {
      ...
      const data = await readJson<(NvetAvailabilitySlot[] & { message?: string })>(response);
      if (!response.ok) throw new Error((data as { message?: string } | null)?.message || 'No se pudo cargar la disponibilidad.');
  ...
  async function handleCreatePet(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = await readJson<NvetPet & { message?: string }>(response);
      if (!response.ok || !data) throw new Error(data?.message || 'No se pudo registrar la mascota.');
  ...
  async function handleBooking(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = await readJson<NvetCreatedAppointment & { message?: string }>(response);
      if (!response.ok || !data) throw new Error(data?.message || 'No se pudo crear la cita.');
      setCreated(data);
    } catch (cause) {
      ...
      <main className="min-h-screen bg-[#F2F4F7] px-4 py-16 sm:px-8">
        <div className="mx-auto max-w-xl rounded-3xl border border-[#34B27A]/20 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-[#5B6670]">
            {created.serviceType} · {date} a las {time}
          </p>
          ...
          <Link
            href="/nvetcareapp/dashboard"
            className="mt-6 inline-flex rounded-xl bg-[#0D1B2A] px-5 py-3 text-sm font-semibold text-white hover:bg-[#172B3F]"
          >
            Ver mis citas
          </Link>
        </div>
  ...
  return (
    <main className="min-h-screen bg-[#F2F4F7] px-4 py-10 sm:px-8 sm:py-14">
      <div className="mx-auto max-w-6xl">
        ...
        {error && (
          <div role="alert" className="mb-5 rounded-xl border border-[#B91C1C]/20 bg-[#B91C1C]/5 px-4 py-3 text-sm text-[#B91C1C]">
            {error}
          </div>
        )}
          ...
          <section className="space-y-6">
            <div className="rounded-2xl border border-black/5 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2">
                <PawPrint className="h-5 w-5 text-[#34B27A]" />
                <h2 className="font-semibold text-[#0D1B2A]">1. Mascota</h2>
              </div>
              {pets.length > 0 && (
                <div className="mb-5 grid gap-2 sm:grid-cols-2">
                  {pets.map((pet) => (
                    <button
                      ...
                      className={`rounded-xl border px-3 py-3 text-left text-sm transition ${selectedPetId === pet.id ? 'border-[#34B27A] bg-[#34B27A]/5' : 'border-black/10 hover:border-[#34B27A]/40'}`}
                    >
              ...
              <form onSubmit={handleCreatePet} className="border-t border-black/5 pt-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <input type="date" max={localDate()} value={petBirthDate} onChange={(event) => setPetBirthDate(event.target.value)} aria-label="Fecha de nacimiento" className="rounded-lg border border-black/10 px-3 py-2 text-sm sm:col-span-2" />
                </div>
                <button disabled={creatingPet} className="mt-3 inline-flex items-center gap-2 rounded-lg border border-[#0D1B2A]/15 px-3 py-2 text-xs font-semibold text-[#0D1B2A] disabled:opacity-60">
                  {creatingPet && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
                  Guardar mascota
                </button>
              </form>
            </div>
            ...
            <div className="rounded-2xl border border-black/5 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2">
                <Stethoscope className="h-5 w-5 text-[#34B27A]" />
                <h2 className="font-semibold text-[#0D1B2A]">2. Veterinario</h2>
              </div>
              ...
              <div className="max-h-[460px] space-y-2 overflow-y-auto pr-1">
                ...
                {vets.map((vet) => (
                  <button
                    key={vet.id}
                    type="button"
                    onClick={() => void selectVet(vet.id)}
                    className={`w-full rounded-xl border p-3 text-left transition ${selectedVetId === vet.id ? 'border-[#34B27A] bg-[#34B27A]/5' : 'border-black/10 hover:border-[#34B27A]/40'}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-[#0D1B2A]">{vetName(vet)}</p>
                        <p className="mt-0.5 text-xs text-[#5B6670]">{vet.city || 'Atención Nvet Care'}{vet.yearsExperience ? ` · ${vet.yearsExperience} años de experiencia` : ''}</p>
                      </div>
          ...
          <section>
            <form onSubmit={handleBooking} className="sticky top-6 rounded-2xl border border-black/5 bg-white p-5 shadow-sm">
              <div className="mb-5 flex items-center gap-2">
                <CalendarDays className="h-5 w-5 text-[#34B27A]" />
                <h2 className="font-semibold text-[#0D1B2A]">3. Servicio y horario</h2>
              </div>
              ...
              {!selectedVet ? (
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    {loadingPrices ? (
                      <p className="flex items-center gap-2 text-sm text-[#5B6670]"><Loader2 className="h-4 w-4 animate-spin" /> Cargando servicios…</p>
                    ) : prices.length === 0 ? (
                      <p className="text-sm text-[#5B6670]">Este veterinario no tiene servicios activos publicados.</p>
                    ) : (
                      <div className="grid gap-2 sm:grid-cols-2">
                        {prices.map((price) => (
                          <button
                            ...
                            className={`rounded-xl border p-3 text-left ${selectedPriceId === price.id ? 'border-[#34B27A] bg-[#34B27A]/5' : 'border-black/10'}`}
                          >
                            <span className="block text-sm font-semibold text-[#0D1B2A]">{price.serviceName}</span>
                            <span className="text-xs text-[#5B6670]">{formatCop(price.priceCop)}</span>
                          </button>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="text-xs font-semibold text-[#333A40]">
                      Fecha
                      <input type="date" min={localDate()} value={date} onChange={(event) => void selectDate(event.target.value)} disabled={!selectedPriceId} className="mt-1 w-full rounded-lg border border-black/10 px-3 py-2 text-sm font-normal disabled:bg-[#F2F4F7]" />
                    </label>
                    <div>
                      <p className="text-xs font-semibold text-[#333A40]">Hora disponible</p>
                      <div className="mt-1 min-h-10 rounded-lg border border-black/10 p-2">
                        {loadingSlots ? <Loader2 className="h-4 w-4 animate-spin text-[#5B6670]" /> : (
                          <div className="flex flex-wrap gap-1.5">
                            {slots.filter((slot) => slot.available).map((slot) => (
                              <button key={slot.time} type="button" onClick={() => setTime(slot.time)} className={`rounded-md px-2 py-1 text-xs ${time === slot.time ? 'bg-[#34B27A] text-white' : 'bg-[#F2F4F7] text-[#0D1B2A]'}`}>{slot.time}</button>
                            ))}
                            {date && !loadingSlots && slots.filter((slot) => slot.available).length === 0 && <span className="text-xs text-[#5B6670]">Sin horarios disponibles.</span>}
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <label className="block text-xs font-semibold text-[#333A40]">
                    <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> Dirección de atención</span>
                    <input required minLength={5} maxLength={250} value={address} onChange={(event) => setAddress(event.target.value)} placeholder="Calle, edificio, apartamento o referencia" className="mt-1 w-full rounded-lg border border-black/10 px-3 py-2 text-sm font-normal" />
                  </label>

                  <label className="block text-xs font-semibold text-[#333A40]">
                    Notas para el veterinario (opcional)
                    <textarea maxLength={500} rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Síntomas, contexto o indicaciones de acceso" className="mt-1 w-full resize-none rounded-lg border border-black/10 px-3 py-2 text-sm font-normal" />
                  </label>
                  ...
                  <button
                    type="submit"
                    disabled={booking || !selectedPetId || !selectedVetId || !selectedPriceId || !date || !time || !address.trim()}
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0D1B2A] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#172B3F] disabled:cursor-not-allowed disabled:opacity-45"
                  >
