...
import { updateNvetGovernanceVetStatus } from '@/lib/nvetcareapp/governance';
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await updateNvetGovernanceVetStatus(accessToken, id, body.isActive, body.reason.trim());
  if (!result.ok) {
    return NextResponse.json(
      { message: result.status === 403 ? 'Gobernanza reservada al SUPERADMIN' : 'No se pudo actualizar el estado del veterinario' },
