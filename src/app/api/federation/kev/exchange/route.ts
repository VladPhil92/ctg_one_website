...
import {
  ...
  isValidPkceVerifier,
  KEV_FEDERATION_PROVIDER,
  pkceChallengeForVerifier,
  sha256Hex,
} from '@/lib/federation/kev';
...
const SERVICE_RATE_LIMIT_SCOPE = 'federation.kev.exchange';
const SERVICE_RATE_LIMIT_ACTOR = 'kev';
...
function noStoreJson(body: unknown, status: number, extraHeaders: HeadersInit = {}) {
...
export async function POST(request: Request) {
  ...
  const rateRow = (Array.isArray(rateData) ? rateData[0] : rateData) as ServiceRateLimitRow | null;
  if (!rateRow) {
    return noStoreJson({ error: 'FEDERATION_RATE_LIMIT_UNAVAILABLE' }, 503);
  }
  ...
  try {
    ...
    if (!parsed) return noStoreJson({ error: 'INVALID_EXCHANGE_REQUEST' }, 400);
    body = parsed;
  } catch (error) {
  ...
  const { data, error } = await admin
    ...
    .eq('provider', KEV_FEDERATION_PROVIDER)
  ...
  return noStoreJson({
    provider: KEV_FEDERATION_PROVIDER,
